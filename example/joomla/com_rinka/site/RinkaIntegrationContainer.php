...
class RinkaIntegrationContainer {
    ...
    public function __construct() {
        ...
        if (!$configuration) {
            ...
            if (isset($configuration['defaultCities'])) {
                $this->defaultCities = $configuration['defaultCities'];
            }
    ...
    public function getDefaultCities() {
        ...
        if (isset($this->domainCityRelation[$domain])) {
            ...
            foreach ($cities as $city) {
                $result[] = array('lietuva', $city);
            }
        } elseif (count($this->defaultCities) > 0) {
            foreach ($this->defaultCities as $city) {
                if (!is_array($city)) {
                    $city = array('lietuva', $city);
                }
                if (count($city) === 2) {
                    $result[] = $city;
                }
    ...
    protected function getDefaultFilter() {
        ...
        foreach ($this->getDefaultCities() as $city) {
            try {
                $filter->addCity($city);
